...
def main():
    ...
    ver_file = op.join('bidsutils', 'info.py')
    ...
    version = None
    cmdclass = {}
    if op.isfile(op.join(root_dir, 'bidsutils', 'VERSION')):
        with open(op.join(root_dir, 'bidsutils', 'VERSION')) as vfile:
            version = vfile.readline().strip()
        pkg_data['bidsutils'].insert(0, 'VERSION')
